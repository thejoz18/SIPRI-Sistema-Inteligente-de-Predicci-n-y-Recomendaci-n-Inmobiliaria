...
MODEL_VERSION = "sipri-explicable-v0.1-demo"
...
def estimate_opinion(data: dict, comparables: list[Comparable], amenities: list[Amenity]) -> dict:
    ...
    candidates = [c for c in comparables if c.operation == operation and c.property_type == property_type]
    if not candidates:
        candidates = [c for c in comparables if c.operation == operation]
    if not candidates:
    ...
    for comparable in candidates:
        ...
        price_per_m2 = comparable.price / max(comparable_area, 1)
        ranked.append({"item": comparable, "score": score, "geo": geo, "ppsqm": price_per_m2})
    ranked.sort(key=lambda row: row["score"])
    nearest = ranked[: min(3, len(ranked))]
    ...
    age_factor = max(0.88, 1 - min(data.get("age_years", 0), 30) * 0.002)
    estimate = weighted_ppsqm * target_area * quality_factor * (1 + amenity_factor) * age_factor
    ...
    margin = 0.15 if not all_verified else 0.10
